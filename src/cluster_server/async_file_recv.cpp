...
private:
    ...
    // Coroutine that periodically samples system metrics (CPU, RAM, disk, network).
    // This calls sleep(1) internally, but it's running in the reactor so we
    // use reactor.sleep_for to avoid blocking the event loop, then do
    // a quick blocking sample.
