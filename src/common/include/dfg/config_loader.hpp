...
public:
  ...
  void print_config_warnings(const std::string &service_name,
    ...
    if (loaded_)
      return; // Config loaded fine, nothing to warn about.
    ...
    std::cerr << "\033["
                 ...
                 "═════════╗\033[0m"
              << std::endl;
    std::cerr << "\033[33m║  ⚠  CONFIG NOT LOADED — " << service_name
              << std::endl;
    std::cerr << "\033["
                 "33m╚═════════════════════════════════════════════════════════"
                 "═════════╝\033[0m"
              << std::endl;
    std::cerr << "\033[33m  None of the following config files could be "
                 "loaded:\033[0m"
              << std::endl;
    ...
    for (const auto &d : defaults) {
      std::cerr << "\033[33m    " << d.key << " = " << d.default_value
                << "\033[0m" << std::endl;
    }
    std::cerr << std::endl;
    std::cerr << "\033[33m  To fix: place a valid JSON config file at one of "
                 "the paths above,\033[0m"
              << std::endl;
    std::cerr << "\033[33m  or set individual values via DFG_<KEY> environment "
                 "variables.\033[0m"
              << std::endl;
    std::cerr << std::endl;
  }
...
private:
  ...
  void parse_object(const std::string &s, size_t &pos,
  ...
  void parse_array(const std::string &s, size_t &pos,
                   const std::string &prefix) {
    if (pos >= s.size() || s[pos] != '[')
      return;
    ++pos; // skip '['
...
inline const std::vector<std::string> head_server = {
    "config/head_server.json", "../config/head_server.json",
    "../../config/head_server.json", "/etc/dfg/head_server.json"};
inline const std::vector<std::string> health_checker = {
    "config/health_checker.json", "../config/health_checker.json",
    "../../config/health_checker.json", "/etc/dfg/health_checker.json"};
inline const std::vector<std::string> cluster_server = {
    "config/cluster_server.json", "../config/cluster_server.json",
    "../../config/cluster_server.json", "/etc/dfg/cluster_server.json"};
inline const std::vector<std::string> zookeeper = {
    "config/zookeeper.json", "../config/zookeeper.json",
    "../../config/zookeeper.json", "/etc/dfg/zookeeper.json"};
} // namespace config_paths
