...
using UnhealthyCallback = std::function<void(int server_id, const ServerHealth& health)>;

/// Callback invoked when a server recovers.
using RecoveredCallback = std::function<void(int server_id, const ServerHealth& health)>;
...
public:
    ...
    void record_heartbeat(int server_id, const ServerHealth& health) {
        ...
        bool was_unhealthy = !record.is_healthy && record.last_heartbeat != std::chrono::steady_clock::time_point{};
    ...
    void check_health() {
        ...
        for (auto& [sid, health] : servers_) {
            if (health.last_heartbeat == std::chrono::steady_clock::time_point{}) continue;
            ...
            if (elapsed > heartbeat_timeout_) {
                ...
                if (health.is_healthy && health.missed_heartbeats >= max_missed_heartbeats_) {
    ...
    void register_server(int server_id, const std::string& ip) {
        ...
        if (servers_.find(server_id) == servers_.end()) {
            ...
            std::cout << "Registered server " << server_id << " (" << ip << ")" << std::endl;
        }
    }
    ...
    void set_unhealthy_callback(UnhealthyCallback cb) { unhealthy_cb_ = std::move(cb); }
    void set_recovered_callback(RecoveredCallback cb) { recovered_cb_ = std::move(cb); }
