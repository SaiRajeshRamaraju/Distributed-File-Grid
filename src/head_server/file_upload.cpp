...
private:
    ...
    void print_chunk_distribution(const std::string& filename, 
        ...
        std::cout << std::endl;
        std::cout << "╔══════════════════════════════════════════════════════════════════╗" << std::endl;
        std::cout << "║  Chunk Distribution Status for: " << std::left << std::setw(32) << filename << "║" << std::endl;
        std::cout << "╠════════╤══════════╤════════════════════════╤════════════════════╣" << std::endl;
        std::cout << "║ Chunk  │ Size     │ Server                 │ Status             ║" << std::endl;
        std::cout << "╠════════╪══════════╪════════════════════════╪════════════════════╣" << std::endl;
        ...
        for (const auto& r : results) {
            ...
            if (r.size >= 1024 * 1024) {
                ...
                oss << std::fixed << std::setprecision(1) << (r.size / (1024.0 * 1024.0)) << " MB";
                size_str = oss.str();
            } else if (r.size >= 1024) {
                std::ostringstream oss;
            ...
            std::cout << "║ " << std::left << std::setw(6) << r.chunk_id 
                      << " │ " << std::setw(8) << size_str
                      << " │ " << std::setw(22) << r.server
                      << " │ " << std::setw(18) << status << " ║" << std::endl;
        }

        std::cout << "╚════════╧══════════╧════════════════════════╧════════════════════╝" << std::endl;
        
        double pct = results.empty() ? 0.0 : (100.0 * success_count / results.size());
        std::cout << "Summary: " << success_count << "/" << results.size() 
                  << " replicas stored successfully (" << std::fixed << std::setprecision(1) << pct << "%)" << std::endl;
        std::cout << "Total chunks: " << total_chunks 
                  << " | Replication factor: " << config_replication_factor() << std::endl;
        std::cout << std::endl;
    }
...
public:
    std::vector<ChunkInfo> split_and_store_file(const std::string& filepath, const std::string& filename) {
        ...
        for (auto& p : pending) {
            ...
            all_results.push_back(TransferResult{
                p.chunk_id, p.server, p.size, p.checksum, ok
            });
            if (ok) {
    ...
    void store_metadata(const std::string& filename, const std::vector<ChunkInfo>& chunks) {
