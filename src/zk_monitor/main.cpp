...
public:
  ...
  bool connect() {
    ...
    if (zk_client_->connect()) {
      connected_ = true;
      std::cout << "Connected to ZooKeeper at: " << connection_string_
                << std::endl;
      return true;
    }

    std::cerr << "Failed to connect to ZooKeeper at: " << connection_string_
              << std::endl;
    return false;
  }
  ...
  bool create_node(const std::string &path, const std::string &data,
    ...
    std::cerr << "Failed to create node " << path << ": " << zerror(rc)
              << std::endl;
    return false;
  }
  ...
  bool update_node(const std::string &path, const std::string &data) {
    ...
    std::cerr << "Failed to update node " << path << ": " << zerror(rc)
              << std::endl;
    return false;
  }
  ...
  bool delete_node(const std::string &path) {
    ...
    std::cerr << "Failed to delete node " << path << ": " << zerror(rc)
              << std::endl;
    return false;
  }
...
private:
  void handle_watch_event(int type, int state, const char *path) {
    if (state == ZOO_CONNECTED_STATE) {
      ...
      connected_ = false;
      std::cerr << "ZooKeeper session expired. Attempting to reconnect..."
                << std::endl;
      // Try to reconnect
    ...
    // Handle node events
    if (path) {
      std::cout << "ZooKeeper watch event: type=" << type << ", state=" << state
                << ", path=" << path << std::endl;
    }
  }
};
...
private:
  ...
  bool check_head_server_health(HeadServerInfo &server) {
    ...
    if (time_since_last_check < std::chrono::seconds(5) &&
    ...
    std::string server_key = server.server_id + ":" + server.ip_address + ":" +
                             std::to_string(server.port);

    for (int attempt = 0; attempt < MAX_RETRIES; ++attempt) {
      int sock = socket(AF_INET, SOCK_STREAM, 0);
      if (sock < 0) {
        std::cerr << "Failed to create socket for " << server_key << ": "
      ...
      if (he == nullptr) {
        close(sock);
        std::cerr << "Failed to resolve hostname: " << server.ip_address
                  << std::endl;
        continue;
      }
    ...
    // If we get here, all retries failed
    {
      ...
      if (failures >= 3) {
        ...
        std::cerr << "Server " << server_key << " marked as unhealthy after "
                  << failures << " consecutive failures" << std::endl;
      } else {
        std::cerr << "Temporary failure for server " << server_key << " ("
                  << failures << " failures so far)" << std::endl;
      }
    }
  ...
  void register_monitor() {
    ...
    std::string monitor_data =
        "monitor_id=" + monitor_id + ",start_time=" +
        std::to_string(
  ...
  void discover_head_servers() {
    ...
    for (const auto &server_name : server_children) {
      ...
      if (!server_data.empty()) {
        ...
        std::cout << "Discovered head server: " << server_name << " at "
                  << info.ip_address << ":" << info.port << std::endl;
      }
    }
  }
  ...
  void perform_leader_election() {
    ...
    for (auto &[server_id, info] : head_servers) {
      if (info.is_leader) {
        ...
        if (check_head_server_health(info)) {
          ...
          std::cout << "Current leader " << current_leader
                    << " is still healthy" << std::endl;
          return;
        } else {
          // Leader is unhealthy, demote it
          std::cout << "Current leader " << current_leader
                    << " is unhealthy, starting election" << std::endl;
          ...
          std::string server_data =
              "ip=" + info.ip_address + ",port=" + std::to_string(info.port) +
              ",status=unhealthy" +
    ...
    if (best_candidate) {
      ...
      if (!current_leader.empty() &&
          head_servers.find(current_leader) != head_servers.end()) {
      ...
      std::string leader_data =
          "server_id=" + leader_server_id +
      ...
      std::cout << "New leader elected: " << leader_server_id
                ...
                << "%, Memory: " << best_candidate->memory_usage << "%)"
                << std::endl;
    } else {
      std::cerr
          << "WARNING: No healthy head servers available for leader election!"
          << std::endl;

      // If we have no leader and no healthy servers, try to elect the least
      // unhealthy one
      HeadServerInfo *least_unhealthy = nullptr;
      for (auto &[server_id, info] : head_servers) {
        if (!least_unhealthy || (info.consecutive_failures <
      ...
      if (least_unhealthy) {
        ...
        std::string leader_path = ZK_ROOT_PATH + "/leader";
        std::string leader_data =
            "server_id=" + leader_server_id + ",status=degraded" +
            ",elected_at=" + std::to_string(now.time_since_epoch().count()) +
        ...
        // Perform health checks
        {
          ...
          for (auto &[server_id, info] : head_servers) {
            ...
            if (healthy)
              ...
              health_check_failures_total_.Increment();

            // Update per-server Prometheus gauges
            per_server_status_.Add({{"server", server_id}})
                .Set(healthy ? 1.0 : 0.0);
            per_server_cpu_.Add({{"server", server_id}}).Set(info.cpu_usage);
            per_server_memory_.Add({{"server", server_id}})
                .Set(info.memory_usage);

            if (old_status != info.status) {
              std::cout << "Head server " << server_id
                        << " status changed: " << old_status << " -> "
  ...
  void generate_health_report() {
    ...
    report << "Current Leader: "
           << (leader_server_id.empty() ? "None" : leader_server_id)
           << std::endl;
    ...
    for (const auto &[server_id, info] : head_servers) {
      ...
      report << "  Server: " << server_id << " | Status: " << info.status
    ...
    report << "Healthy Servers: " << healthy_count << "/" << head_servers.size()
           << std::endl;
...
public:
  ZooKeeperHeadServerMonitor(const std::string &zk_connection_string)
    ...
    monitor_id =
        "monitor_" + std::to_string(getpid()) + "_" +
        std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count());
    ...
    std::cout << "Initialized ZooKeeper structure at: " << zk_connection_string
              << std::endl;
  ...
  void simulate_head_server_registration(const std::string &server_id,
    ...
    std::string server_data =
        ...
        std::to_string(10.0 +
                       (std::rand() % 30)) + // Random CPU usage between 10-40%
        ",memory_usage=" +
        std::to_string(
            40.0 + (std::rand() % 50)) + // Random memory usage between 40-90%
        ",active_connections=" +
    ...
    if (zk_client->create_node(server_path, server_data, true)) { // ephemeral
      std::cout << "Simulated head server registration: " << server_id << " at "
                << ip << ":" << port << std::endl;
    } else {
  ...
  void run_interactive_mode() {
    std::cout << "\n=== ZooKeeper Head Server Monitor Interactive Mode ==="
              << std::endl;
    std::cout << "Commands:" << std::endl;
    std::cout << "  register <server_id> <ip> <port> - Register a head server"
              << std::endl;
...
// TODO: This is complete bullshit. Change this to get the LAN / VIRTUAL WAN
...
std::string get_local_ip_address() {
  ...
  // BUG: This is wrong way of doing things. We shouldn't consider internet
  // always available. Use Google's DNS server as a dummy address
  ...
  // TODO: This isn't a good way of doing things. We are using linux, there are
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--zk-hosts" && i + 1 < argc) {
      ...
      std::cout << "Usage: " << argv[0]
                << " [--zk-hosts HOST:PORT] [--interactive]\n"
                << "Options:\n"
                << "  --zk-hosts HOST:PORT  ZooKeeper connection string "
  ...
  std::cout << "╔═══════════════════════════════════════════════════╗"
            << std::endl;
  std::cout << "║   Distributed File Grid - ZooKeeper Monitor      ║"
            << std::endl;
  ...
  std::cout << "  Session Timeout:        "
            << cfg.get_int("zookeeper.session_timeout_ms", 10000) << "ms"
            << std::endl;
  std::cout << "  Monitor Interval:       "
            << cfg.get_int("monitor.interval_seconds", 10) << "s" << std::endl;
  std::cout << "  Heartbeat Timeout:      "
            << cfg.get_int("monitor.heartbeat_timeout_seconds", 30) << "s"
            << std::endl;
  std::cout << std::endl;
  ...
  try {
    ...
    // We use port 9670 because the head server's TCP listener (Control API)
    // runs on 9670.
    g_monitor->simulate_head_server_registration("head_server_1",
                                                 "head-server-1", 9670);
    g_monitor->simulate_head_server_registration("head_server_2",
                                                 "head-server-2", 9670);
